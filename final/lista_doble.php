<?php
require("nodo_doble.php");

class lista
{
	var $ultima_posicion = null;
	var $primer_elemento = null;
	var $ultimo_elemento = null;
	//var $num_elementos = 0;
	
	function length()
	{
		$cont=0;
		$nodo_actual=$this->primer_elemento;
		while($nodo_actual->siguiente!=null)
		{
			$nodo_actual=$nodo_actual->siguiente;	
			$cont++;
		}
		return $cont+1;//devuelve el tamaño de la lista
	}
	
	function insertar_elemento($elemento, $pos)
	{
		if($this->primer_elemento==null)
		{
			$this->primer_elemento=new nodo($elemento, null, null);
		}
		else
		{
			$cont=0;
			
			$nodo_actual=$this->primer_elemento;
			while($cont<$pos-1)
			{
				$nodo_actual=$nodo_actual->siguiente;	
				$cont++;
			}
			$a=new nodo($elemento, $nodo_actual->siguiente,$nodo_actual);
			$nodo_actual->siguiente=$a;
			$nodo_actual->siguiente->anterior=$a;
			//$a->siguiente->anterior=$nodo_actual->siguiente;
		}
	}
	
	function imprimir()
	{
		$nodo_actual=$this->primer_elemento;
		while($nodo_actual!=null)
		{
			echo $nodo_actual->dato."<br>";
			$nodo_actual=$nodo_actual->siguiente;
		}
	}
	
	function recuperar($pos)
	{
		$cont=0;
		$nodo_actual=$this->primer_elemento;
		if(ultima_posicion==null)
		{
			while($nodo_actual!=null)
			{
				$nodo_actual->dato;
				if($cont==$pos)
				{
					echo $nodo_actual->dato."<br>";
				}
				else
				{
					$nodo_actual=$nodo_actual->siguiente;
				}
				$cont++;
			}
			$cont=$ultimo_posicion;
		}
		else
		{
			$dif=($pos -$ultimo_posicion );
			if($dif>0)
			{
				while($nodo_actual!=null)
				{
					$nodo_actual->dato;
					if($cont==$dif)
					{
						echo $nodo_actual->dato."<br>";
					}
					else
					{
						$nodo_actual=$nodo_actual->siguiente;
					}
					$cont++;
				}
				$cont=$ultimo_cont;
			}
			else
			{
				while($nodo_actual!=null)
				{
					$nodo_actual->dato;
					if($cont==$dif)
					{
						echo $nodo_actual->dato."<br>";
					}
					else
					{
						$nodo_actual=$nodo_actual->anterior;
					}
					$cont--;
				}
				$cont=$ultimo_cont;
			}
		}
	}
}
/*

$mi_lista = new lista();
$mi_lista->insertar_elemento("6", 0);
$mi_lista->insertar_elemento("4", 1);
$mi_lista->insertar_elemento("3", 2);
$mi_lista->insertar_elemento("5", 3);
$mi_lista->insertar_elemento("2", 4);
$mi_lista->insertar_elemento("a", 5);
$mi_lista->imprimir();*/
?>